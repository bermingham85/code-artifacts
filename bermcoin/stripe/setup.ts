...
/**
 * BERMCOIN-BCK-CFG-00001 — Stripe Product & Price Setup
 *
 * Creates Bermcoin Standard and Pro products with monthly/annual prices.
...
async function findOrCreateProduct(
  ...
  const existing = await stripe.products.search({
    query: `metadata['plan']:'${plan}' AND metadata['product']:'bermcoin'`,
  });
  ...
  const product = await stripe.products.create({
    name,
    metadata: { plan, product: 'bermcoin' },
  });
...
async function main() {
  console.log('=== Bermcoin Stripe Setup ===\n');

  // Products
  const standardId = await findOrCreateProduct('Bermcoin Standard', 'standard');
  const proId = await findOrCreateProduct('Bermcoin Pro', 'pro');
  ...
  const whCheckout = await findOrCreateWebhook(
    `${N8N_BASE}/webhook/bermcoin-checkout`,
    ['checkout.session.completed'],
    'Bermcoin onboarding — checkout complete'
  );
  const whOffboard = await findOrCreateWebhook(
    `${N8N_BASE}/webhook/bermcoin-offboard`,
    ['customer.subscription.deleted'],
    'Bermcoin offboarding — subscription cancelled'
  );
