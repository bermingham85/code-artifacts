...
/**
 * BERMCOIN-BCK-SCR-00002 — Smoke Test Suite
 *
...
const TEST_EMAIL = `smoke-test-${Date.now()}@bermcoin.test`;
let testMemberId: string | null = null;
const TEST_EMAIL_B = `smoke-test-b-${Date.now()}@bermcoin.test`;
...
async function run() {
  console.log('=== BERMCOIN SMOKE TESTS ===\n');
  ...
  await test(10, 'n8n workflows accessible', async () => {
    try {
      ...
      const data = await res.json();
      const wfs = data?.data || [];
      const bermcoin = wfs.filter((w: any) => w.name?.toLowerCase().includes('bermcoin'));
      return { pass: bermcoin.length >= 5, detail: `Found ${bermcoin.length}/5 Bermcoin workflows` };
    } catch (e: any) {
  ...
  await test(11, 'Stripe prices exist', async () => {
    ...
    try {
      ...
      const data = await res.json();
      const bermcoin = (data?.data || []).filter((p: any) =>
        p.nickname?.includes('standard') || p.nickname?.includes('pro')
      );
      return { pass: bermcoin.length >= 4, detail: `Found ${bermcoin.length}/4 Bermcoin prices` };
    } catch (e: any) {
      return { pass: false, detail: e.message };
    }
  });
