...
- **HunyuanVideo + kohya_ss install scripts (G4, G5)**:
  - `muscle_install_kohya_ss.py` clones `https://github.com/bmaltais/kohya_ss` to `C:/Users/Owner/Tools/kohya_ss/`. Layered verification: **structure_ok** (sd-scripts/library/train_util.py exists on disk) — required, blocks the cert if false. **import_ok** (sd-scripts library imports cleanly) — DEFERRED by WO §3 ("optional per-character LoRA training"); script exposes the deferred state in its return JSON but treats structure_ok as the cert-relevant success signal. Operator reruns the script without `--skip-deps` (which triggers `pip install -r sd-scripts/requirements.txt`) when LoRA training is actually needed. Idempotent: if dir exists, runs `git pull` then re-verifies.
  - `muscle_install_hunyuanvideo.py` clones `https://github.com/kijai/ComfyUI-HunyuanVideoWrapper` to `C:/ComfyUI/custom_nodes/ComfyUI-HunyuanVideoWrapper/`, runs `pip install -r requirements.txt` using the **active Python interpreter** (this box's ComfyUI runs against system Python 3.13 at `C:\Program Files\Python313\python.exe`; there is no separate ComfyUI venv. The script honours `--python <path>` to override when a venv is introduced). Lists the required weight files with HuggingFace URLs gated behind an explicit `--download-weights` flag (operator gate on the multi-GB pull; verification of nodes-only install does not require weights).
