...
| `work gate blocked Claude/Codex loop startup` | Repo is stale, on a protected branch, dirty without acknowledgement, or cannot prove latest remote state | Resolve blocker, or inspect dirty local changes and rerun with `--allow-dirty-work-gate` only for write continuation | Loop starts only after gate JSON is not `BLOCK` |
